...
impl idl::InOrderAttestImpl for AttestServer {
    ...
    fn quote(
        ...
        nonce_src.read_range(0..32, &mut nonce[..]).map_err(|_| RequestError::Fail(ClientError::WentAway))?;
        hasher.update(nonce);
        ...
        Ok(dest.write_range(0..dest.len(), &self.buf[0..len])
            .map_err(|_| RequestError::Fail(ClientError::WentAway))?)
    }
    ...
    fn quote_len(
        ...
        let len = hubpack::serialize(self.buf, &signature)
            .map_err(|_| AttestError::SerializeSignature)?;
        let len = u32::try_from(len).map_err(|_| AttestError::SignatureTooBig)?;
